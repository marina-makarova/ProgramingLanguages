...
(* expressions in a little language for 2D geometry objects values: points,
   lines, vertical lines, line segments other expressions: intersection of two
   expressions, lets, variables, (shifts added by you) *)
...
datatype geom_exp = 
	 ...
	 | Var of string
   | Shift of real * real * geom_exp (* shift by deltaX, deltaY *)
...
fun intersect (v1,v2) =
  ...
  (* First compute the intersection of (1) the line containing the segment and
     (2) v2. Then use that result to compute what we need. *)
        ...
        | LineSegment seg2 => 
          ...
          (* the hard case in the hard case: seg and seg2 are on the same line
             (or vertical line), but they could be (1) disjoint or (2)
             overlapping or (3) one inside the other or (4) just touching.  And
             we treat vertical segments differently, so there are 4*2 cases. *)
...
fun shift (dX, dY, v) =
  case v of
    ...
    | Line (slope, inter) => Line (slope, inter + dY - (slope * dX))
...
(* interpreter for our language: takes a geometry expression and returns a
    geometry value for simplicity we have the top-level function take an
    environment, (which should be [] for the whole program we assume the
    expression e has already been "preprocessed" as described in the homework
    assignment: line segments are not actually points (endpoints not real close)
...
fun preprocess_prog geom_exp = 
  case geom_exp of
    ...
    | LineSegment (x1,y1,x2,y2) => 
        ...
        then Point (x1,y1)
        else if (not (real_close(x1,x2)) andalso x1 > x2) orelse 
                ((real_close(x1,x2)) andalso not (real_close(y1,y2)) andalso y1 > y2)
